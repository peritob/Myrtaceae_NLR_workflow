...
#to run this script: run_BNL.sh interproscan_out.tsv augustus_out.gff3 > species_BNL.gff3

# to get a list of gene ids to make a fasta

gawk 'BEGIN {FS="\t"} $5=="PF00931" { print $1 }' $1 | sort -k 1b,1 | uniq > tempP
gawk 'BEGIN {FS="\t"} $5=="PF02892" { print $1 }' $1 | sort -k 1b,1 | uniq > tempQ
gawk 'BEGIN {FS="\t"} ($5=="G3DSA:3.80.10.10" || $5=="PF08263" || $5=="PF13516"  || $5=="PF12799" || $5=="PF13306" || $5=="PF13855") { print $1 }' $1 | sort -k 1b,1 | uniq > tempR
join tempP tempQ > tempS
join tempR tempS > BNL.list
...
rm tempP tempQ tempR tempS tempT
