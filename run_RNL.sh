...
#to run this script: run_RNL.sh interproscan_out.tsv augustus_out.gff3 > species_RNL.gff3

# to get a list of gene ids to make a fasta

gawk 'BEGIN {FS="\t"} $5=="PF00931" { print $1 }' $1 | sort -k 1b,1 | uniq > tempF
gawk 'BEGIN {FS="\t"} $5=="PF05659" { print $1 }' $1 | sort -k 1b,1 | uniq > tempG
gawk 'BEGIN {FS="\t"} ($5=="G3DSA:3.80.10.10" || $5=="PF08263" || $5=="PF13516"  || $5=="PF12799" || $5=="PF13306" || $5=="PF13855") { print $1 }' $1 | sort -k 1b,1 | uniq > tempH
join tempF tempG > tempI
join tempI tempH > RNL.list
...
rm tempF tempG tempH tempI tempJ
